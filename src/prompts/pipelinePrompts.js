...
- If the detail is missing or unclear, leave it out instead of guessing.
...
• Vehicle names:
  ...
  - If you are unsure, leave the detail out. Do NOT guess.
...
Use these triggers only (NO extra categories, NO guessing):
...
- RECON_APPOINTMENT: service/RWC/tint/tyres/body/interior/keys/mechanical type appointments (use context).
- SOLD: car is sold.
...
CRITICAL:
- Use ONLY information explicitly present in the line (and obvious model→make hints).
- Do NOT guess rego, make, model, or year if the line does not support it.
- If something is missing, leave it as "".
`;
...
Rules:
- Only output SOLD actions if the text clearly describes a car being sold.
- Do NOT assume a car is sold if the text doesn’t say so.
`;
...
- readiness: short phrase summarizing the ready state (e.g., "ready for pickup").
- Only mark READY when the text clearly says a specific car is ready.
`;
...
- Do NOT assume extra pickups or destinations that are not written.
`;
...
- name: person coming (customer) if explicitly named, else "".
- dateTime: as stated ("today at 3pm", "next Tuesday", etc). Do NOT normalize to absolute dates.
- notes: any extra context ("bring out car", "second viewing", etc).
...
Your job here:
- For EACH input line, output AT LEAST ONE RECON_APPOINTMENT action.
- Use ONLY the text of that line plus the user-configured category keywords below.
- Do NOT invent cars, regos, or appointments that are not supported by the text.
...
- Use ONLY the user-configured entries below. A category matches if ANY of its "keywords" OR "rules" (case-insensitive substring) appear in the text for that line.
...
User-configured categories (for matching, read-only):
...
- nextLocation: the intended future location (e.g., "Al's", "front lot").
- Only output actions when next destination intent is clearly described.
`;
...
- task: short description of the job (e.g., "get Car 6789 detailed", "order relay for GTI AC").
- Only output TASK actions for real tasks. Do NOT create extra cars or jobs.
`;
